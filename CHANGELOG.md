...
- Synchronous and asynchronous `Model.upsert()` builders with mapping values,
  primary or unique conflict targets, explicit existing-row updates, typed
  `returning()` results, and native PostgreSQL and SQLite execution.

### Changed

- Bulk `insert()` and `upsert()` mappings are now used exactly as supplied.
  SQLARec identifier generation is limited to `create()` and
  `create_with_session()`.
